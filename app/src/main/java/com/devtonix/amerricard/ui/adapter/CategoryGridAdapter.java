...
public class CategoryGridAdapter extends RecyclerView.Adapter<CategoryGridAdapter.MainHolder> {
    ...
    private List<CardItem> favorites;
    ...
    public CategoryGridAdapter(Context context, List<CardItem> items, OnFavoriteClickListener listener, int width, int height) {
...
//        favorites = FavoriteUtils.getFavorites();
    ...
    public void onBindViewHolder(final MainHolder holder, int position) {
        ...
        String url = NetworkModule.BASE_URL + item.getType() + "/" + item.getId() + "/image?width=" + width + "&height=" + height + "&type=fit";

        Glide.with(context).load(url)
                .into(holder.icon);
    }

    private boolean isFavorite(CardItem item) {
//        for (CardItem i : favorites) {
...
//    private void manageFavorites(CardItem item) {
//        if (isFavorite(item)) {
